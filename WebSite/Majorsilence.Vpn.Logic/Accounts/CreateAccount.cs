...
public class CreateAccount
{
    ...
    public int Execute()
    {
        ...
        using (var db = InitializeSettings.DbFactory)
        {
            ...
            using (var txn = db.BeginTransaction())
            {
                ...
                var userid = db.Insert(new Poco.Users(
                    details.Email,
                    pwd.Password,
                    pwd.Salt,
                    details.Firstname,
    ...
    {
        var subject = string.Format("{0} Account Created", Helpers.SiteInfo.SiteName);
        ...
        email.SendMail_BackgroundThread(message.ToString(), subject, details.Email, true, null);
    }
    ...
    private void BetaKeySetup()
    {
        ...
        {
            db.Open();
            var info = db.Query<Majorsilence.Vpn.Poco.BetaKeys>("SELECT * FROM BetaKeys WHERE Code = @code",
                new { code = details.BetaKey });
            if (info.Count() != 1)
                throw new Exceptions.InvalidBetaKeyException(
                    "There appears to have been an error validating the beta key.");

            betaKey = info.First();
            if (betaKey.IsUsed) throw new Exceptions.BetaKeyAlreadyUsedException("The beta key has already been used.");
        }
    }

    private bool RequiresBetaKey()
    {
        if (Helpers.SiteInfo.LiveSite == false) return true;
    ...
    private void ValidateData()
    {
        ...
        if (details.Email.Trim() == "") throw new Exceptions.InvalidDataException("An email address must be filled in");

        if (details.Password.Trim() == "") throw new Exceptions.PasswordLengthException("A password must be filled in");

        if (details.Firstname.Trim() == "") throw new Exceptions.InvalidDataException("A first name must be filled in");

        if (details.Lastname.Trim() == "") throw new Exceptions.InvalidDataException("A last name must be filled in");
    ...
    private void ValidateEmailNotAlreadyUsed()
    {
        ...
        {
            db.Open();
            var info = db.Query<Majorsilence.Vpn.Poco.BetaKeys>("SELECT * FROM Users WHERE Email = @Email",
