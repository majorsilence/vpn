...
public class InitialDatabase : Migration
{
    public override void Up()
    {
        ...
        Insert.IntoTable("DatabaseInfo").Row(new
            { VersionId = "1", CreateTime = DateTime.UtcNow, LastDailyProcess = DateTime.UtcNow });

        Insert.IntoTable("LookupPaymentType").Row(new
            { Code = "MONTHLY", Description = "Monthly subscription.  Payments made once a month." });
        Insert.IntoTable("LookupPaymentType").Row(new
            { Code = "YEARLY", Description = "Yearly subscription.  Payments made once a year." });

        // PaymentRates
        Insert.IntoTable("PaymentRates").Row(new { CurrentMonthlyRate = 9.97m, CurrentYearlyRate = 99.97m });

        // Counters
        Insert.IntoTable("Counters").Row(new
            { Code = "VPNCERT", Description = "Current vpn cert count.  Used to make unique vpn cert names", Num = 1 });

        // Regions
        var usRegionId = 1;
        var canadaRegionId = 2;
        ...
        // VpnServers
        Insert.IntoTable("VpnServers").Row(new
        {
            Address = "127.0.0.1", VpnPort = 1194,
            Description = "default vagrant testing vpn authority", RegionId = canadaRegionId, Active = true
        });
        Insert.IntoTable("VpnServers").Row(new
        {
            Address = "10.0.2.2", VpnPort = 1194,
            Description = "default vagrant testing vpn authority, access from vagrant web site",
            RegionId = canadaRegionId, Active = true
        });
        ...
        var sshPassword = "aPassword";

        Insert.IntoTable("SiteInfo").Row(new
        {
